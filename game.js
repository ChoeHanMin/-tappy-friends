...
const CHARACTERS = {
  ...
  leo: {
    name: '레오', desc: '수수께끼의 황금 냥사자 · 올라운더',
...
const ACHIEVEMENTS = [
  ...
  { id: 'hidden_unlocked', icon: '🦁', title: '미스터리 발견', desc: '숨겨진 캐릭터 레오 잠금 해제',
    check: () => isLeoUnlocked() },
...
function startBgm() {
  ...
  bgmInterval = setInterval(() => {
    if (muted) return;
    beep(BGM_NOTES[bgmStep % BGM_NOTES.length], 0.32, 'sine', 0.045);
    bgmStep++;
  }, 380);
}
...
function gameOver() {
  ...
  updateLeaderboard({
    score, distance: distFloor, char: selectedChar,
...
function renderLeaderboardList() {
  ...
  leaderboardListEl.innerHTML = list.map((e, i) => {
    const name = (CHARACTERS[e.char] && CHARACTERS[e.char].name) || e.char;
    return `<div class="lb-row">
      <div class="lb-rank">${medals[i] || (i + 1)}</div>
      <div class="lb-body">
        <div class="lb-name">${name} · 점수 ${e.score}</div>
